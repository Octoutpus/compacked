...
local Versions = "69.420"
local Cmd = {}

Cmd[#Cmd + 1] = {Text = "versions "..Versions,Title = "Script versions"}
Cmd[#Cmd + 1] = {Text = "outdated :skull:",Title = "script's kinda outdated due to uhhhhhhhhhhhhhhhhhhhh prison life updating owo"}
Cmd[#Cmd + 1] =	{Text = "cmd / cmds",Title = "Show commands"}
...
Cmd[#Cmd + 1] =	{Text = "crash / skidcr",Title = "Lag server and disconnect after 5 minutes of lagging"}
Cmd[#Cmd + 1] =	{Text = "uncrash / unskidcr",Title = "Stop lag server"}
...
Cmd[#Cmd + 1] =	{Text = "crashp / beam3 [plr]",Title = "Shoot a beam to player but even more lag"}
...
Cmd[#Cmd + 1] =	{Text = "!getprefix",Title = "If you forget prefix you can type this or whatever"}
...
game.StarterGui:SetCore("SendNotification", {
Title = "Script remodeled by 2qg5";
Text = "abuse this if you are retarded";
Duration = 7;
})
end
...
function PlayerChatted(Message)
	...
	if Command("crashp") or Command("beam3") then
		local Player = GetPlayer(Arg2)
		if Player then
	...
	if Command("crash") or Command("skidcr") then
		States.Lag_Server = true
		Notify("Lagging server", Color3.fromRGB(215, 2, 2), "Success")
		while wait() do
	...
	if Command("uncrash") or Command("stopskidcr") or Command("unskidcr") or Command("stopcrash") then
...
function AdminPlayerChatted(Message, Player)
	...
	if Command("cmd") or Command("cmds") then
		...
		Chat("/w "..Player.Name.." "..Prefix.."spamarrest [plr] "..Prefix.."unspamarrest "..Prefix.."skidcr "..Prefix.." "..Prefix.."beam [plr]")
